  ...
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fuwudanhao" label="服务单号" prop="fuwudanhao">
              <el-input v-model="ruleForm.fuwudanhao" 
                placeholder="服务单号" readonly></el-input>
          </el-form-item>
        </div>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.fuwuxiangqing = this.ruleForm.fuwuxiangqing.replace(reg,'../../../ssmmggaw/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fuwuliebiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fuwuliebiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
